...
class TaHomaDevice extends IPSModule
{
    ...
    private function RegisterVariables(): void
    {
        ...
        if (!empty($data)) {
            ...
            $this->SetupVariable(
                'ControlShutter', $this->Translate('Control'), '~ShutterMoveStop', $this->_getPosition(), VARIABLETYPE_INTEGER, true, true
            );
    ...
    function RequestStatus()
    {
        $data = json_decode($this->SendDataToParent(json_encode([
            ...
            'Endpoint' => self::DEVICE . $this->ReadPropertyString('DeviceID'),
            'Payload' => ''
        ])), true);
    ...
    function CheckResponse($data)
    {
        ...
        if (isset($devices['fault'])) {
            $this->SendDebug('Tahoma Error', $devices['fault']['faultstring'], 0);
            $this->SendDebug('Tahoma Error Detail', $devices['fault']['detail']['errorcode'], 0);
        }
        return $check;
    }
    ...
    public function Open()
    {
        ...
        $this->SetValue('ControlShutter', self::OPEN);
    }
    ...
    public function SendCommand(string $name)
    {
        $result = json_decode($this->SendDataToParent(json_encode([
            'DataID' => '{656566E9-4C78-6C4C-2F16-63CDD4412E9E}',
            'Endpoint' => self::DEVICE . $this->ReadPropertyString('DeviceID') . self::EXEC,
            'Payload' => json_encode([
                'name' => $name,
                'parameters' => []
            ])
    ...
    public function Stop()
    {
        ...
        $this->SetValue('ControlShutter', self::STOP);
    }

    public function Close()
    {
        $this->SendCommand('close');
        $this->SetValue('ControlShutter', self::CLOSE);
    }
    ...
    public
    function SetLowSpeedMode(bool $value)
    {
    ...
    public function ReceiveData($JSONString)
    {
        ...
        $devices = $data->Buffer;
        $this->SendDebug('Receive devices state', $devices, 0);
        if ($devices != '[]') {
            $devices = json_decode($devices, true);
            foreach ($devices as $device) {
                $device_id = $this->ReadPropertyString('DeviceID');
                // $this->SendDebug('devices id', $device['id'], 0);
                if ($device_id == $device['id']) {
                    $this->SendDebug('found devices id', $device_id, 0);
                    $data = $device['states'];
                    if (count($data) > 0) {
                        foreach ($data as $state) {
                            if ($state['name'] === 'position') {
                                $this->SetValue('position', $state['value']);
                                $this->SendDebug('Set value position to', $state['value'], 0);
    ...
    public
    function SetWebFrontVariable(string $ident, bool $value)
    {
        ...
        if ($check != false) {
            $form = [
                ...
                [
                    'type' => 'ValidationTextBox',
                    'enabled' => false,
                    'value' => $this->Translate($this->ReadPropertyString('Type')),
                    'caption' => 'Type:'
                ]
            ];
        } else {
