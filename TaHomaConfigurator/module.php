...
class TaHomaConfigurator extends IPSModule
{
    ...
    {
        $devices =  json_decode($this->ReadAttributeString('Devices'));
        $devices = json_decode(json_encode($devices), true);
        if(empty($devices))
        {
            $devices = $this->GetConfiguration();
        }
        return $devices;
    }
    ...
    private function RequestDataFromParent(string $endpoint)
    {
        ...
        $this->SendDebug('Tahoma Devices Response', json_encode($devices), 0);
        return $devices;
    }

    public function GetConfiguration()
    {
        $devices = $this->RequestDataFromParent(self::SITE . $this->ReadPropertyString('SiteID') . self::DEVICES);
        $devices = json_decode(json_encode($devices), true);
        if(isset($devices[0]['name']))
        {
            foreach ($devices as $device) {
                $this->SendDebug('Tahoma Site', $device['name'] . ", " . $device['type'], 0);
            }
        }
        else{
            $devices = [];
    ...
    protected function FormHead()
    {
        $form = [
            ...
            [
                'type' => 'Label',
                'caption' => 'category for Somfy devices'
            ],
            [
                'name' => 'ImportCategoryID',
                'type' => 'SelectCategory',
                'caption' => 'category Somfy devices'
            ],
            [
                'name' => 'TaHomaConfiguration',
                'type' => 'Configurator',
                'caption' => 'Somfy devices:',
                'visible' => true,
                'rowCount' => 20,
                'add' => false,
                'delete' => false,
    ...
    protected function FormActions()
    {
        $form = [
            [
                'type' => 'Label',
                'visible' => true,
                'caption' => 'Read Somfy devices:'
            ],
            [
                'type' => 'Button',
                'visible' => true,
                'caption' => 'Read devices',
                'onClick' => 'TAHOMA_GetConfiguration($id);'
            ]
        ];
        return $form;
    }
