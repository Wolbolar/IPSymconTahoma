...
class TaHomaDiscovery extends IPSModule
{
    ...
    {
        $sites =  json_decode($this->ReadAttributeString('Sites'));
        $sites = json_decode(json_encode($sites), true);
        if(empty($sites))
        {
            $sites = $this->GetConfiguration();
        }
        return $sites;
    }
    ...
    private function RequestDataFromParent(string $endpoint)
    {
        ...
        $this->SendDebug('Tahoma Site Response', json_encode($sites), 0);
        return $sites;
    }

    public function GetConfiguration()
    {
        $sites = $this->RequestDataFromParent(self::ALL_SITES);
        $sites = json_decode(json_encode($sites), true);
        if(isset($sites[0]['id']))
        {
            foreach ($sites as $site) {
                if(isset($site['id']))
                {
                    $this->SendDebug('Tahoma Site', $site['label'] . " " . $site['id'], 0);
                }
            }
        }
        else{
    ...
    {
        return $this->ReadAttributeString('Sites');
    }

    /**
     * Liefert alle Sites.
     *
     * @return array configlist all sites
     */
    ...
    protected function FormHead()
    {
        ...
        $form = [
            ...
            [
                'name' => 'TaHomaConfiguratorConfiguration',
                'type' => 'Configurator',
                'caption' => 'Somfy Sites:',
                'visible' => true,
                'rowCount' => 10,
    ...
    protected function FormActions()
    {
        $form = [
            [
                'type' => 'Label',
                'visible' => true,
                'caption' => 'Read Somfy configuration:'
            ],
            [
                'type' => 'Button',
                'visible' => true,
